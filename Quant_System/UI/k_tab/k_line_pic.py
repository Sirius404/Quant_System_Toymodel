...
class KMplCanvas(FigureCanvas):
    ...
    def start_staict_plot(self, df, method="CDLDOJISTAR", numb=0):
        ...
        # df['K'], df['D'] = talib.STOCH(df['high'].values, df['low'].values, df['close'].values, fastk_period=9,
        #                                slowk_period=3,
        #                                slowk_matype=0, slowd_period=3, slowd_matype=0)
        # df['K'].dropna(0, inplace=True)
        # df['D'].dropna(0, inplace=True)
        # df['J'] = 3 * df['K'] - 2 * df['D']
